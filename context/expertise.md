---
last_updated: 2026-04-06
source_refs:
  - blogs/building-products-age-of-ai.md
  - blogs/unlocking-revenue-with-product-led-growth.md
  - blogs/glutenornot-free-ingredient-scanner-celiac-disease.md
  - blogs/how-we-brought-3dprinteros-to-life.md
  - blogs/3d-printing-101-class.md
---

# Expertise & Knowledge

## Product Management (Deep)

Core professional skill across multiple companies (Bond, Teachable, ManyChat). Hands-on experience with:
- Product-led growth (PLG) — designed and shipped self-service features that generated $1M+ revenue
- User research and prototyping — runs customer interviews, builds InVision/Figma prototypes, tests with users
- PRDs, go-to-market plans, stakeholder buy-in presentations
- Cross-functional leadership — works across engineering, sales, marketing, design
- Onboarding and activation flows
- Mobile and web product development

## AI Tooling & Prompt Engineering (Growing)

Active practitioner, not theorist. Uses AI tools daily for building:
- Claude Code for development — migrations, new projects, debugging
- Prompt engineering as product logic (GlutenOrNot's celiac knowledge prompt)
- MCP servers — built personal context MCP, experimented with Strava MCP
- ChatGPT for personal productivity — recipes, travel planning, tech questions
- Vibe coding with Replit, Claude Code, and other tools

## Chatbot & Messaging Platforms (Professional)

Current work at ManyChat. Understands:
- Messaging automation platforms
- Chat-based customer engagement
- Bot-building workflows and user experience

## 3D Printing & Hardware (Historical, Deep)

Earlier career focused on 3D printing at scale:
- Managing fleets of 3D printers
- Teaching 3D printing courses
- Manufacturing applications and hardware operations
- Wrote extensively about the industry (7 blog posts)

## Robotics & Physical Products (Hands-on)

Bond/Wami experience with handwriting robots:
- Production operations — running a robot fleet in a Bushwick studio
- Supply chain and logistics — buying, moving, and setting up physical assets
- Hardware maintenance and automation

## Online Creator Economy & EdTech

Teachable experience gives insight into:
- Online course platforms and creator tools
- Revenue models for creators
- Platform growth and user engagement

## Web Development (Practical, Not Professional)

Not an engineer, but builds and ships:
- Astro static sites (aaronroy.com)
- React Native mobile apps (GlutenOrNot)
- Python scripts and automation (scheduled-agents, ingest tools)
- Streamlit apps (fantasy Tour de France)
- WordPress migrations, deployment (Vercel)

## Cybersecurity (Personal Interest)

Wrote practical guides on:
- Two-factor authentication setup
- Online scam prevention
- Privacy protection for non-technical audiences
